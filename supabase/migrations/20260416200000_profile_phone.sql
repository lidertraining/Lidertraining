-- ============================================================
-- LiderTraining — Telefone obrigatório no cadastro
-- ============================================================
-- Adiciona profiles.phone (E.164 digits). Permite ao líder falar
-- direto com sua downline via WhatsApp (wa.me/<phone>).
-- ============================================================

alter table profiles
  add column if not exists phone text;

-- Permitir ao usuário atualizar o próprio telefone
revoke update on profiles from authenticated;
grant update (name, avatar_url, comm_goal, onboarded, fir_step, fir_completed, streak_freeze_active, phone) on profiles to authenticated;

-- ------------------------------------------------------------
-- signup_with_invite: agora recebe p_phone (obrigatório, 10+ dígitos)
-- ------------------------------------------------------------
create or replace function signup_with_invite(p_code text, p_name text, p_phone text)
returns profiles
language plpgsql
security definer
...
-- ------------------------------------------------------------
-- get_team_learning: agora inclui phone
-- ------------------------------------------------------------
